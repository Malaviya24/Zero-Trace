...
import { getAudioWatchdogAction } from "@/lib/call-watchdog";
...
function GroupCallPageContent() {
  ...
  const callPreflightError = CONFIG.callPreflight.canStartCalls
    ? null
    : CONFIG.callPreflight.missingTurnReason ||
      "Calls are unavailable because TURN relay credentials are missing.";
  ...
  useEffect(() => {
    ...
    createNewCall();
  }, [isNew, createCallMutation, navigate, callPreflightError, roomIdHint]);
  ...
  const resetNegotiationHealth = useCallback(
    (participantId: string) => {
      ...
      stateForPeer.audioRecoveryStep = 0;
      stateForPeer.lastAudioRecoveryAt = 0;
      stateForPeer.audioFailureNotified = false;
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      for (const participant of participants) {
        ...
        const now = Date.now();
        const hasRemoteAudio = !!peerConn?.remoteStream?.getAudioTracks().some((track) => track.readyState === "live");
        const isConnected =
          !!peerConn?.remoteStream ||
          peerConn?.pc.connectionState === "connected" ||
          peerConn?.pc.iceConnectionState === "connected";
        if (isConnected) {
          const firstSeenConnected = peerFirstSeenAtRef.current.get(participant._id) ?? now;
          peerFirstSeenAtRef.current.set(participant._id, firstSeenConnected);
          const action = getAudioWatchdogAction({
            isConnected,
            hasRemoteAudio,
            firstSeenAt: firstSeenConnected,
            now,
            audioRecoveryStep: stateForPeer.audioRecoveryStep,
            lastAudioRecoveryAt: stateForPeer.lastAudioRecoveryAt,
          });

          if (action === "none" && hasRemoteAudio) {
            resetNegotiationHealth(participant._id);
            continue;
          }

          if (action === "ice-restart") {
            stateForPeer.audioRecoveryStep = 1;
            stateForPeer.lastAudioRecoveryAt = now;
            void runOfferAttemptRef.current(participant._id, "audio-watchdog-ice-restart", { iceRestart: true });
            continue;
          }

          if (action === "hard-reset") {
            stateForPeer.audioRecoveryStep = 2;
            stateForPeer.lastAudioRecoveryAt = now;
            hardResetPeerConnection(participant._id, "audio-watchdog-reset");
            void runOfferAttemptRef.current(participant._id, "audio-watchdog-recover");
            continue;
          }

          if (action === "open-circuit") {
            stateForPeer.circuitOpen = true;
            clearOfferRetryTimer(participant._id);
            if (!stateForPeer.audioFailureNotified) {
              stateForPeer.audioFailureNotified = true;
              toast.error("Audio path is unstable for one participant. Use Reconnect in People panel.");
            }
            continue;
          }
          continue;
        ...
        const firstSeen = peerFirstSeenAtRef.current.get(participant._id) ?? now;
        peerFirstSeenAtRef.current.set(participant._id, firstSeen);
        if (now - firstSeen < 6000) continue;

        if (peerConn?.hasRemoteDescription && now - firstSeen > 12000) {
